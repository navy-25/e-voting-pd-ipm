...
my ${CC} = q[cl];
my ${LIBS} = q[];
my ${installbuilddir} = q[C:\Xampp\apache\build];
my ${APRUTIL_LIB_TARGET} = q[];
my ${bindir} = q[C:\Xampp\apache\bin];
my ${APRUTIL_SO_EXT} = q[dll];
my ${LD} = q[link];
my ${CPP} = q[cl -nologo -E];
my ${LDFLAGS} = q[ kernel32.lib /nologo /subsystem:windows /dll /machine:I386 ];
my ${includedir} = q[C:\Xampp\apache\include];
my ${exec_prefix} = q[C:\Xampp\apache];
my ${datadir} = q[C:\Xampp\apache];
my ${APRUTIL_LIBNAME} = q[libaprutil-1.lib];
my ${libdir} = q[C:\Xampp\apache\lib];
